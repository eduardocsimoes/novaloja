<div class="product_item">
	<a href="<?php echo BASE_URL; ?>product/open/<?php echo $id; ?>">
		<div class="product_tags">
			<?php if($sale == 1){ ?>
				<div class="product_tag product_tag_red"><?php $this->language->get('SALE'); ?></div>
			<?php } ?>
			<?php if($bestseller == 1){ ?>
				<div class="product_tag product_tag_green"><?php $this->language->get('BESTSELLER'); ?></div>
			<?php } ?>
			<?php if($new_product == 1){ ?>
				<div class="product_tag product_tag_blue"><?php $this->language->get('NEW'); ?></div>
			<?php } ?>
		</div>
		<div class="product_image">
			<img src="<?php echo BASE_URL; ?>media/products/<?php echo $images[0]['url']; ?>" width="100%">
		</div>
		<div class="product_name">
			<?php echo $name; ?>
		</div>
		<div class="product_brand">
			<?php echo $brand_name; ?>
		</div>
		<div class="product_price_from">
			<?php 
				if($price != '0'){
					echo 'R$. '.number_format($price_from, 2, ',','.');
				}
			?>
		</div>
		<div class="product_price">
			R$ <?php echo number_format($price, 2, ',','.'); ?>
		</div>
		<div style="clear:both"></div>
	</a>
</div>